...
async function syncdb() {
  ...
  let AstaConn = AstaConnectSock({
    ...
    browser: ["Windows", "chrome", "𝕀𝕋𝔸ℂℍ𝕀-𝕄𝔻-𝕡𝕚𝕩𝕖𝕝"],
  ...
  AstaConn.ev.on("connection.update", async (_0x3fd0b3) => {
    ...
    if (_0x2224a7 === "open") {
      ...
      let _0x1f88ec =
        ...
        Config.WORKTYPE +
        "\n  Database: " +
        (isMongodb ? "MongoDb" : sqldb ? "PostegreSql" : "ITACHI-MD-pixel") +
        "\n";
      try {
  ...
  AstaConn.messageId = (length = 8, prefix = "𝕀𝕋𝔸ℂℍ𝕀-𝕄𝔻-𝕡𝕚𝕩𝕖𝕝") => {
...
app.listen(randomPort, () =>
  console.log(
    "𝕀𝕋𝔸ℂℍ𝕀-𝕄𝔻-𝕡𝕚𝕩𝕖𝕝 Server listening on http://localhost:" + randomPort + "/ "
  )
);
...
async function MakeSession(
  ...
  const officialRepoPath = "/ITACHI-MD-pixel/";
